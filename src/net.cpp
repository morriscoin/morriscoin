...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "MorrisCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
       ...
       {"seed.morrisco.in", "seed.morrisco.in"},
    // {"morriscoinpool.org", "dnsseed.morriscoinpool.org"},
...
void ThreadOpenAddedConnections(void* parg)
{
    ...
    // Make this thread recognisable as the connection opening thread
    RenameThread("morriscoin-opencon");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. MorrisCoin is probably already running."), addrBind.ToString().c_str());
        else
